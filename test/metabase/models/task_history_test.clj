...
(defn- add-sec
  [t]
  (time/plus t (time/secs 1)))

(defn- add-10-millis
  [t]
  (time/plus t (time/millis 10)))

(defn- make-10-millis-task [started_at]
  {:started_at (du/->Timestamp started_at)
   :ended_at   (du/->Timestamp (add-10-millis started_at))})
...
(let [task-4 (tu/random-name)
  ...
  (expect
    ...
    (let [t1-start (time/now)
          t2-start (add-sec t1-start)
          t3-start (add-sec t2-start)
          t4-start (add-sec t3-start)
          t5-start (add-sec t4-start)]
