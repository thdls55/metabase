...
(s/defn calculate-duration :- s/Int
...
(s/defn ^:private store-sync-summary
  ...
  (db/insert-many! TaskHistory
    (cons {:task operation
          ...
          (for [[step-name {:keys [start-time end-time duration] :as step-info}] steps
                :let [task-details (dissoc step-info :start-time :end-time :duration :log-summary-fn)]]
            {:task step-name
             :db_id (u/get-id database)
             :started_at (du/->Timestamp start-time)
             :ended_at (du/->Timestamp end-time)
